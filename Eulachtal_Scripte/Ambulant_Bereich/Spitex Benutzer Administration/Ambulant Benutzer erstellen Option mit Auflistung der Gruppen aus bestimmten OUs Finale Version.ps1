...
$btnCreate.Add_Click({
    ...
    try {
        New-ADUser -SamAccountName $samAccountName -UserPrincipalName "$samAccountName@eulachtal.zh" -GivenName $givenName -Surname $surname -Name "$givenName $surname" -EmailAddress $email -Path $OU -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -Enabled $true | Out-Null
